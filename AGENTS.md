...
- **Crash/Stuck Analysis**: If the recompiler hangs or produces invalid code:
   - Use `--trace` to log every instruction analyzed: `./build/bin/gcrecomp roms/gamecom_rom.bin --trace`
   - Use `--limit <N>` to stop after N instructions to prevent infinite loops (useful for finding where analysis gets stuck): `./build/bin/gcrecomp roms/gamecom_rom.bin --limit 10000`
